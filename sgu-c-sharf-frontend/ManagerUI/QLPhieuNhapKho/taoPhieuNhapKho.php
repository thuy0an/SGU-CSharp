                    ...
                    <div style="padding-left: 16%; width: 100%; padding-right: 2rem">
                        <div class="wrapper">
                            ...
                            <div class="boxFeature d-flex flex-wrap" style="gap: 2rem;" id="boxFeature">
                                <select class="form-control" id="supplierSelect" style="width: 35%; padding: 10px 0;"></select>
...
<script>
    ...
    function loadSuppliers() {
        ...
        $.ajax({
            url: `${window.env.API_ROOT}/suppliers`, // Replace with your actual API endpoint
            type: 'GET',
            dataType: 'json',
            headers: {
            ...
            success: function(response) {
                // Assuming the API returns an array of supplier objects with 'id' and 'name' properties
                var supplierSelect = document.getElementById('supplierSelect');
                supplierSelect.innerHTML = '<option value="">Chọn nhà cung cấp</option>'; // Add a default option

                response.data.content.forEach(function(supplier) {
                    var option = document.createElement('option');
                    option.value = supplier.id; // Use the supplier ID as the value
                    option.text = supplier.name; // Display the supplier name
    ...
    function handleSubmit() {
        ...
        // Get the selected supplier object (you might need to store this data when loading the suppliers)
        const token = sessionStorage.getItem("token");

        $.ajax({
            url: `${window.env.API_ROOT}/suppliers/${selectedSupplierId}`, // Replace with your actual API endpoint
            type: 'GET',
            dataType: 'json',
            headers: {
                'Authorization': 'Bearer ' + token
            ...
            success: function(response) {
                var selectedSupplier = response.data
                var sodienthoainhacungcap = selectedSupplier.phone;
                ...
                formData.append("totalAmount", totalValue);
                ...
                $.ajax({
                    ...
                    url: `${window.env.API_ROOT}/inventory-report`, // Cập nhật URL nếu cần
            ...
            error: function(xhr, status, error) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Lỗi',
                    text: 'Không thể tải thông tin nhà cung cấp.',
                });
            }
        });
    }
